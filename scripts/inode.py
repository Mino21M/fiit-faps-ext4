...
class inode():
    ...
    def parseInode(self, chunk):
        ...
        self.blocks = self.getInodeType(chunk)
    ...
    def parse_extent(self, byte, block):
    ...
    def getBlocksExtentTree(self, chunk):
        ...
        for block in range(entries_following_header):
            position = 12*(block+1)
            if depth == 0:
                blocks.append(self.parse_extent(chunk[position:position+12], block))
            else:
    ...
    def getInodeType(self, chunk):
        ...
        if self.magic_number == 0xF30A:
            blocks = self.getBlocksExtentTree(chunk)
        """
        else:
            blocks = []
            for i in range(end):
